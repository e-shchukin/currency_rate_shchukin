...
public class CurrencyService : ICurrencyService
{
    ...
    private List<(Currency, Rate)> MapDtoToEntities(List<(CurrencyDto Currency, RateDto Rate)> rates)
    {
        return rates.Select(x =>
        {
            var currencyEntity = x.Currency.Adapt<Currency>();
            var rateEntity = x.Rate.Adapt<Rate>();
